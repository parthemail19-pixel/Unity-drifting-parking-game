...
namespace WakaTime {
  ...
  public class Plugin {
    ...
    // Throttle for activity-based heartbeats (selection / scene-view interaction).
    private static double _lastActivityHeartbeatTime;
    private const double ACTIVITY_HEARTBEAT_INTERVAL = 30;
    ...
    public static void Initialize() {
      ...
      if (EditorPrefs.HasKey(API_KEY_PREF)) {
        var rawKey = EditorPrefs.GetString(API_KEY_PREF);
        Debug.Log("<HackaTime> raw key len=" + rawKey.Length + " first=" + (rawKey.Length > 0 ? (int) rawKey[0] : -1) + " last=" + (rawKey.Length > 0 ? (int) rawKey[rawKey.Length - 1] : -1));
        _apiKey = rawKey.Trim().Trim('\0');
      }

      if (EditorPrefs.HasKey(API_URL_PREF)) {
        _apiUrl = EditorPrefs.GetString(API_URL_PREF).Trim().Trim('\0');
      }
      ...
      Debug.Log("<HackaTime> project name = [" + ProjectName + "]");
    ...
    // Fires a heartbeat when the user is actively working in the editor
    // (selecting objects, or dragging / key-navigating / scrolling in the Scene view),
    // throttled to one per ACTIVITY_HEARTBEAT_INTERVAL seconds so continuous work
    // accrues time without spamming. Only real input triggers this, so walking away
    // stops the heartbeats and idle time is never counted.
    static void OnEditorActivity() {
      var now = EditorApplication.timeSinceStartup;
      if (now - _lastActivityHeartbeatTime < ACTIVITY_HEARTBEAT_INTERVAL) return;
      _lastActivityHeartbeatTime = now;
      SendHeartbeat();
    }
    ...
    static void OnSceneViewGui(SceneView sceneView) {
      var e = Event.current;
      if (e == null) return;
      if (e.type == EventType.MouseDown || e.type == EventType.MouseDrag ||
          e.type == EventType.KeyDown || e.type == EventType.ScrollWheel) {
        OnEditorActivity();
      }
    }

    // Moving / renaming / organizing files in the Project window.
    static void OnProjectChanged() {
      OnEditorActivity();
    }

    // Moving objects, editing transforms/values, adding components, etc.
